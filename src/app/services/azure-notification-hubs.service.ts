...
// Interface(s)
import { Registration, Notification, EventType } from '../models/events';
...
export class AzureNotificationHubsService {
  ...
  constructor(private alertCtrl: AlertController, private platform: Platform) {
    ...
    this.platform.ready().then(() => {
      ...
      // make sure the config values are set in the environments.ts file.
      if (environment.hubName && environment.hubConnectionString) {
        // Initialize the Push Service
        const push = PushNotification.init({
          // Initialize the ANH Cordova plugin, this is required to use ANH
          // Pass in our configuration values
          notificationHubPath: environment.hubName,
          connectionString: environment.hubConnectionString,
        ...
        PushNotifications.addListener(
          ...
          (pushNotification: PushNotification) => {
            console.log(
              'AzureNotificationHubsService: pushNotificationReceived'
            );
        ...
        // Tell the user this won't work until they fix the environment.
        this.alertCtrl
          .create({
            header: 'Invalid Configuration',
            // tslint:disable-next-line:max-line-length
            message:
              "Please populate the project's <strong>src/app/environment.ts</strong> file with settings from your Azure Notification Hubs configuration.",
